...
int L1_access_latency;
int L2_cache_size;
int L2_block_size;
int L2_associativity;
int L2_access_latency;
...
public:
	int tag;
	bool dirty;
	int participation_num;// the nth block participating
	block(){
		participation_num = 0;
		tag = -1;
		dirty = false;
...
public:
	...
	int size_keeper; // monitors if capacity of set is full (equal to block size)
	int set_size;/**/
	int block_size;
	sets_of_cache(int a, int b){ // associtivity is number of blocks in a set,	associativyt and blocks size in order
		set_size = 0;
		size_keeper = 0;
		block_size = b;
		Blocks.resize(a); // making number of blocks in a set
...
class cache{
public:
	std::vector<sets_of_cache> cache_sets;
	int block_size;
	int access_latency;
	int associativity;
	int num_of_sets;
	int num_of_blocks;
	int cache_size;
	int num_of_blocks_in_set; // this and associativity are same
	int new_participant_num;
	cache(int a, int b, int c, int d){//associativity, block size, cache size, time_latency in same order has to be given
		new_participant_num = 0;
		associativity = a;
		block_size = b;
		cache_size = c;
		access_latency = d;
		num_of_blocks = cache_size/block_size;
		num_of_sets = num_of_blocks/associativity;
		num_of_blocks_in_set = num_of_blocks/num_of_sets;
		// we need to form no. of sets
		cache_sets.resize(num_of_sets);
		for(long long unsigned int i=0;i<cache_sets.size();i++){
			sets_of_cache temp(num_of_blocks_in_set, block_size);
			cache_sets[i] = temp;
		}
	}
	void Insert(int address){		// we do this when we say load or at store
		new_participant_num++; // for knowing participants_num we have to increse even if just accessing and assign it to the block

		int set_index_and_tag = address/block_size;
		//int offset = address % block_size;
		int set_index = set_index_and_tag%num_of_sets;
		int tag = set_index_and_tag/num_of_sets;			//tag,set_index,offset we have to give addresses to blocks
		// if(set_size/*sat class member */ == associativity)  make sure of LRU else no need

		if(cache_sets[set_index].size_keeper<associativity){
			for(long long unsigned int index = 0; index<cache_sets[set_index].Blocks.size();index++){
				if(cache_sets[set_index].Blocks[index].tag==-1){
					cache_sets[set_index].Blocks[index].tag = tag;
					cache_sets[set_index].Blocks[index].participation_num = new_participant_num;
					break;
				}
			}
			cache_sets[set_index].size_keeper++;
		}
		else{//LRU
			int replacable_index;
			replacable_index = 0;
			for(long long unsigned int index = 1;index<cache_sets[set_index].Blocks.size();index++){
				if(cache_sets[set_index].Blocks[replacable_index].participation_num < cache_sets[set_index].Blocks[index].participation_num){
					replacable_index = index;
				}
			}
			//int temp = replacable_index;
			cache_sets[set_index].Blocks[replacable_index].participation_num = new_participant_num;
			cache_sets[set_index].Blocks[replacable_index].tag = tag;

			// we are considering to make a write through so we are not using dirty bit here however we may need to use in advancement of the project

		}

		//if set size not full normal insert at place , if place occupied LRU (you can keep wherever in the set)
	...
	bool Search(int address){
		...
		for(long long unsigned int index = 0; index < cache_sets[set_index].Blocks.size();index++){
			if(tag == cache_sets[set_index].Blocks[index].tag){
				cache_sets[set_index].Blocks[index].participation_num = new_participant_num;
				cache_hit = true;
				break;
			}
		...
		/*
			Plac3e for the search code
		*/

		//since we are doing write through we are not using dirty bit if the block here
		return cache_hit;
	}
	...
	block* getBlock(int address){
		...
		return temp;
				...
				/*
					1. search address if found in L1 (/L2) leave if it's hit with :
								(i) just increase the pipelingin diag.
								(ii)keep about how recently it is used (we have to keep about when it's last used like if some has least then it;s least used )
					2. if not hit search then set may or may not be filled completely( we have to do a way to find out set entries)
					3. if set is not filled then then insert and keep a age bit of it for least recently used type
					4. Now the set contains either way

					5. there might be a hit in L2 but not in L1, so we have to make sure we search in both L1 and L2 in the function
							if found in L1 we have to make sure we get a block from memory or address for it (maybe keep a function getblock addresses or no need I guess)
				*/
cache L1(L1_associativity,L1_block_size,L1_cache_size,L1_access_latency);
cache L2(L2_associativity,L2_block_size,L2_block_size,L2_access_latency);
...
void jump(string str) {
	for (long long unsigned int i = 0; i < text_label_array.size(); i++) {
...
void memory_stage(string opcode,int rd, string operand1, string operand2,int offset, int* result, bool* t1, bool* t2) {
	if (opcode == "lw") {
		*t1 = false;
		*t2 = false;
		int address = 4*(*result);
		if(L1.Search(address)){
			*t1 = true;
		}
		else{
			L1.Insert(address);
		}
		if(L2.Search(address)){
			*t2 = true;
		}
		else{
			L2.Insert(address);//make sure this gets a aged number(particioation number)
		}
...
int main() {
    cout << "Enter the file name for cache L1 details(with file path and file extension) " << endl;
    string L1_file;
    getline(cin, L1_file);
    fstream cache_file1;
    cache_file1.open(L1_file, ios::in | ios::out);
    if(!cache_file1.is_open()){
        cout << "Error in opening the L1 cache file";
    }
    else{//associativity, block_size, cache_size, latency
        cout << "L1 CACHE file OPENED successfully" << endl;
        string line;
        int i =0;
        while(cache_file1.good()){
            getline(cache_file1,line);
            if(i==0){
				L1_associativity = stoi(line);
            }
            else if(i==1){
				L1_block_size = stoi(line);
            }
            else if(i==2){
				L1_cache_size = stoi(line);
            }
            else if(i==3){
				L1_access_latency = stoi(line);
            }
			cache temp(L1_associativity, L1_block_size, L1_cache_size, L1_access_latency);
			L1 = temp;
        }
    }
    cache_file1.close();
    cout << "Enter the file name for cache L2 details(with file path and file extension) " << endl;
    string L2_file;
    getline(cin, L2_file);
    fstream cache_file2;
    cache_file2.open(L2_file, ios::in | ios::out);
    if(!cache_file2.is_open()){
        cout << "Error in opening the L2 cache file";
    }
    else{//associiativity, block size, cache size, access latency in this order for every line
        cout << "L2 CACHE file OPENED successfully" << endl;
        string line;
        int i =0;
        while(cache_file2.good()){
            getline(cache_file2,line);
            if(i==0){
				L2_associativity = stoi(line);
            }
            else if(i==1){
				L2_block_size = stoi(line);
            }
            else if(i==2){
				L2_cache_size = stoi(line);
            }
            else if(i==3){
				L2_access_latency = stoi(line);
            }
			cache temp(L2_associativity, L2_block_size, L2_cache_size, L2_access_latency);
			L2 = temp;
        }
    }
    cache_file2.close();

	int memory_access_latency;
	cout << "Enter the value of the memory access latency for the RAM using : " ;
	cin >> memory_access_latency;
	...
	if (!file.is_open())
		...
		for (; program_counter<vec.size();) {
			...
			if (start_clock > pipeline_table[number_of_performed_instructs].size() - 100) {//if in case of unavailabel clocks
				pipeline_table[number_of_performed_instructs].resize(100);
			}
			...
			if (instruction == "") {
				continue;
								...
								//performing pipelining for the instruction_fetch stage
			...
			stage_number++;
						...
						//performing instruction decode in pipelining register
			...
			int rt = -1;
			int result = -1;
			reg_fetch(opcode, operand1, operand2, operand3, &rd, &rs, &rt, &offset, &result);//offset will be corresponding to the index of the memory array
			stage_number++;
			temp.rd = rd;
			temp.rs = rs;
			temp.rt = rt;
			...
			if (number_of_performed_instructs>0 && (pipeline_table[number_of_performed_instructs - 1][start_clock + ptr].ID_RF == true && pipeline_table[number_of_performed_instructs - 1][start_clock + ptr].stall == true)) {//in case of stalls occured whil prefoeming ID_RF to prev instruction at same cycle
				for(int j = start_clock + ptr; j>=0; j++) {//present instruction will be stalls if the prev instruction is in same stage or being stalls
			...
			bool L1_hit = false;
			bool L2_hit = false;
			memory_stage(opcode, rd, operand1, operand2, offset, &result, &L1_hit, &L2_hit);
			// we need to know somehow if there is a hit in L1 and L2 caches - maybe pass the caches into memory stage function
			stage_number++;
			int run_cycles;
			...
			//pipeline_table[number_of_performed_instructs][start_clock + ptr] = temp;
			if(number_of_performed_instructs == 0){
				for(int cycle_runner = 0; cycle_runner < L1_access_latency+L2_access_latency+memory_access_latency;cycle_runner++){
					pipeline_table[number_of_performed_instructs][start_clock+ptr+cycle_runner] = temp;
				}
				ptr += (L1_access_latency+L2_access_latency+memory_access_latency);
			}
			else{
				// if above cycle is occupied with memory stage , now this will result in stall
				for(int stall_maker = start_clock+ptr; pipeline_table[number_of_performed_instructs-1][stall_maker].WB!=true; ptr++){
					temp.stall = true;
					pipeline_table[number_of_performed_instructs][stall_maker] = temp;
					if(pipeline_table[number_of_performed_instructs-1][stall_maker].stall==false){
						num_stalls++;
					}
					stall_PC.push_back(number_of_performed_instructs+1);
					stall_instructions.push_back(instruction);
					//increase the *stalls* number if above is not stall and throw this into *stall_instruction*
				}
				temp.stall = false;

				int cycle_runner;// if above cycles are not occupied with memory
				for(int cycle_runner = 0; cycle_runner<run_cycles;cycle_runner++){
					pipeline_table[number_of_performed_instructs][start_clock+ptr+cycle_runner] = temp;
				}
				ptr += cycle_runner;
			}
			...
			// we may need to remove old pipelining and keep new one saying it's sum of latencies for 1st time and memory cyles become stallls until prev instruct gets into WB stage
			// we also have to think of replacing bits maybe we make sure every time like this
				// after every address generated we have to make sure :
				/*
					1. search address if found in L1 (/L2) leave if it's hit with :
								(i) just increase the pipelingin diag.
								(ii)keep about how recently it is used (we have to keep about when it's last used like if some has least then it;s least used )
